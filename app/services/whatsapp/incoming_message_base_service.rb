...
class Whatsapp::IncomingMessageBaseService
  ...
  def set_conversation
    Rails.logger.info "=== EVOLUTION DEBUG ==="
    Rails.logger.info "ContactInbox ID: #{@contact_inbox.id}"

    Rails.logger.info "Conversas do contato:"
    Rails.logger.info @contact_inbox.conversations.pluck(:id, :status).inspect

    open_conversation = @contact_inbox.conversations.where.not(status: :resolved).last

    Rails.logger.info "Conversa aberta encontrada:"
    Rails.logger.info(open_conversation&.id)
    ...
    Rails.logger.info "Conversa escolhida:"
    Rails.logger.info(@conversation&.id)

    return if @conversation

    Rails.logger.info "CRIANDO NOVA CONVERSA"
